...
from app.presentation.serializers.authentication import (
...
from app.presentation.serializers.users import UserResponseSerializer
...
def register_user(request: Request) -> Response:
    ...
    user = register_user_rule(**validated_data, event=UserVerificationEmailEvent)
...
def verify_email_request(request: Request) -> Response:
    ...
    request_email_verification_rule(
        email=serializer.validated_data["email"], event=UserVerificationEmailEvent
    )
...
def login_user(request: Request) -> Response:
    ...
    user, tokens = login_user_rule(**validated_data, event=UserUpdateEvent)
...
def logout_user(request: Request) -> Response:
    ...
    logout_user_rule(**validated_data, event=UserLogoutEvent)
...
def complete_social_authentication(request: Request, backend_name: str) -> Response:
    ...
    user, tokens = social_authentication_rule.complete_authentication(
        request, event=UserUpdateEvent
    )
